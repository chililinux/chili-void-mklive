...
var ExtensionLoader = function(vFs, cb) {
  ...
  var setSandboxApi = function(sandbox, tools) {
    !engine.apiCache && (engine.apiCache = tools.getApiTemplate());
    ...
    sandbox.sendMessage({action: 'setUILanguage', data: uiLanguage});
    sandbox.sendMessage({action: 'setURL', url: chrome.runtime.getURL("")});
    sandbox.sendMessage({action: 'chromeApi', api: engine.apiCache});
  ...
  var Extension = function(vFs, vTextFs, acceptLangs) {
    ...
    tools.initSandbox(engine, function(sandbox) {
