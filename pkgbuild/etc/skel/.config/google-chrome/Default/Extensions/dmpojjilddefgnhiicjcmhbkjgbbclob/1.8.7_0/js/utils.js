...
utils.checkUpdate = function(cb) {
  ...
  var xhr = new XMLHttpRequest();
  var page = 'https://addons.opera.com/en/extensions/details/app_id/' + appId + '/';
  xhr.open('GET', page, true);
  xhr.onload = function() {
    var text = xhr.responseText;

    if (!/<dt>Version<\/dt>/.test(text)) {
      return cb({
        status: 'error',
        text: 'Extension page is not found!'
      });
    }

    var r = new RegExp(utils.safeRegexp('<dt>Version</dt><dd>'+version+'</dd>'));
    if (r.test(text)) {
      return cb({
