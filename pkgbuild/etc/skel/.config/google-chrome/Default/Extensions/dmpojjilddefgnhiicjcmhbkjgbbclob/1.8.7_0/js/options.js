...
var writeDoc = function(body, engine) {
  ...
  var apiArray = tools.apiCacheToArray(engine.apiCache);

  content = content.replace('{api async list}', apiArray.filter(function(item) {
    if (item[2] !== '[Function]') {
      return false;
    }
    return [
  ...
  content = content.replace('{api var list}', apiArray.filter(function(item) {
    if (item[2] === '[Function]') {
      return false;
    }
    return [
